...
class CNN:
    def __init__(self, classifier_type, is_load=True):
        ...
        self.model_saved_path = cfg.model_saved
        self.classifier_type = classifier_type
        if is_load:
    ...
    def build_model(self, epoch_num, is_compile=True):
        ...
        self.model = Dense(128, activation='relu', kernel_regularizer='l2')(self.model)
        self.model = Dropout(0.5)(self.model)
        self.model = Dense(3, activation='softmax', kernel_regularizer='l2')(self.model)

        self.model = Model(inputs=base_model.input, outputs=self.model)
        if is_compile:

            self.model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
            self.model.fit(self.data.X_train, self.data.Y_train, batch_size=128, epochs=epoch_num, verbose=1,
                           validation_data=(self.data.X_test, self.data.Y_test))

    def train(self, data, epoch_num=10):
        self.data = data
        self.model_saved_path = cfg.model_saved
        self.build_model(epoch_num)
        self.model.save(self.model_saved_path)
        print("Trained model is saved to", self.model_saved_path)
