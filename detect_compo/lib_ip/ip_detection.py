...
def is_symmetric(bbox, compo_area, symmetric_similarity, mask_copy):
    mask = mask_copy.copy()
    col_min, row_min, col_max, row_max = bbox.put_bbox()
    col_min, row_min, col_max, row_max = col_min + 1, row_min + 1, col_max + 1, row_max + 1
    max_similarity = 0
    horizontal = [(col_min, col_max), (col_min + 1, col_max), (col_min, col_max - 1)]
    for i, j in horizontal:
        count = 0
        while i <= j:
            for row in range(row_min, row_max + 1):
                if mask[row][i] + mask[row][j] == 2:
                    count += (1 + int(i != j))
            i += 1
            j -= 1
        if count / compo_area >= symmetric_similarity:
            return True
        # else:
        #     max_similarity = max(count / compo_area, max_similarity)
    vertical = [(row_min, row_max), (row_min + 1, row_max), (row_min, row_max - 1)]
    for i, j in vertical:
        count = 0
        while i <= j:
            for col in range(col_min, col_max + 1):
                if mask[i][col] + mask[j][col] == 2:
                    count += (1 + int(i != j))
            i += 1
            j -= 1
        if count / compo_area >= symmetric_similarity:
...
def is_rectangle(bbox, similarity, mask_copy):
    mask = mask_copy.copy()
    col_min, row_min, col_max, row_max = bbox.put_bbox()
    col_min, row_min, col_max, row_max = col_min + 1, row_min + 1, col_max + 1, row_max + 1
    perimeter = (row_max - row_min + col_max - col_min - 2) * 2
    up = np.sum(mask[row_min, col_min:col_max + 1])
    down = np.sum(mask[row_max, col_min:col_max + 1])
    left = np.sum(mask[row_min + 1:row_max, col_min])
    right = np.sum(mask[row_min + 1:row_max, col_min])
    rectangle_similarity = (up + down + left + right) / perimeter
    return False if rectangle_similarity < similarity else True


def is_slim(bbox, threshold, mask_copy):
    mask = mask_copy.copy()
    col_min, row_min, col_max, row_max = bbox.put_bbox()
    col_min, row_min, col_max, row_max = col_min + 1, row_min + 1, col_max + 1, row_max + 1
    horizontal = np.sum(mask[row_min:row_max + 1, col_min:col_max + 1], axis=1)
    if np.sum(horizontal <= 2) / bbox.height >= threshold:
        return True
    vertical = np.sum(mask[row_min:row_max + 1, col_min:col_max + 1], axis=0)
    if np.sum(vertical <= 2) / bbox.width >= threshold:
        return True
    return False
...
def component_detection(org, binary, min_obj_area,
    ...
    compo_idx = 0
    # os.makedirs('../../data/volvo/noise', exist_ok=True)
    # os.makedirs('../../data/volvo/button', exist_ok=True)
    # binary = cv2.morphologyEx(binary, cv2.MORPH_OPEN, (1, 1))
    ...
    for i in range(0, row, 3): 
        for j in range(i % 2, column, step_v):
            if binary[i, j] == 255 and mask[i, j] == 0:
                ...
                if component.bbox_area == 0: continue
                ...
                compo_idx += 1
                # a, b, c, d = random.randint(-2, 0), random.randint(-2, 0), random.randint(0, 2), random.randint(0, 2)
                # col_min, row_min, col_max, row_max = component.put_bbox()
                # col_min, row_min, col_max, row_max = col_min +  a, row_min + b, col_max + c, row_max + d
                # img = org[max(row_min, 0):row_max + 1, max(col_min, 0):col_max + 1]
                if ff[0] / component.bbox_area < max_area_ratio:
                    symmetric, rectangle = is_symmetric(component, ff[0], 0.75, mask_copy), is_rectangle(component, 0.75, mask_copy)
                    if symmetric or rectangle:
                        if not is_slim(component, 0.75, mask_copy) or rectangle:
                            # cv2.imwrite('../../data/volvo/noise/loniq' + str(idx) + '_' + str(compo_idx) + '_ture.jpg', img)
                            # cv2.imwrite('../../data/volvo/noise/loniq' + str(idx) + '_' + str(compo_idx) + '_original.jpg', mask_copy * 255)
                            compos_all.append(component)
                        else:
                            # cv2.imwrite('../../data/volvo/noise/loniq' + str(idx) + '_' + str(compo_idx) + '_slim1.jpg', img)
                            # cv2.imwrite('../../data/volvo/noise/loniq' + str(idx) + '_' + str(compo_idx) + '_original.jpg', mask_copy * 255)
                            compos_all.append(component)
                    else:
                        # cv2.imwrite('../../data/volvo/noise/loniq' + str(idx) + '_' + str(compo_idx) + '_zfalse.jpg', img)
                        # cv2.imwrite('../../data/volvo/noise/loniq' + str(idx) + '_' + str(compo_idx) + '_original.jpg', mask_copy * 255)
                        compos_all.append(component)
                else:
                    if ff[0] > 50 or not is_slim(component, 0.75, mask_copy):
                        # cv2.imwrite('../../data/volvo/button/loniq' + str(idx) + '_' + str(compo_idx) + '.jpg', img)
                        # cv2.imwrite('../../data/volvo/button/loniq' + str(idx) + '_' + str(compo_idx) + '_original.jpg', mask_copy * 255)
                        compos_all.append(component)
                    else:
                        # cv2.imwrite('../../data/volvo/noise/loniq' + str(idx) + '_' + str(compo_idx) + '_slim2.jpg', img)
                        # cv2.imwrite('../../data/volvo/noise/loniq' + str(idx) + '_' + str(compo_idx) + '_original.jpg', mask_copy * 255)
                        compos_all.append(component)
